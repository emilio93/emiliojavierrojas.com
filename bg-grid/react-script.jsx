class BgBoxComponent extends React.Component {
  constructor(props) {
    ...
    const off1 = [20, 20, 0];
    const rgb1 = [
      [hex1[0] - off1[0], hex1[0] + off1[0]],
      [hex1[1] - off1[1], hex1[1] + off1[1]],
      [hex1[2] - off1[2], hex1[2] + off1[2]],
    ];

    const hex2 = [228, 228, 228];
    const off2 = [0, 0, 0];
    const rgb2 = [
      [hex2[0] - off2[0], hex2[0] + off2[0]],
      [hex2[1] - off2[1], hex2[1] + off2[1]],
      [hex2[2] - off2[2], hex2[2] + off2[2]],
    ];

    const hex3 = [200, 72, 72];
    const off3 = [0, 20, 20];
    const rgb3 = [
      [hex3[0] - off3[0], hex3[0] + off3[0]],
      [hex3[1] - off3[1], hex3[1] + off3[1]],
      [hex3[2] - off3[2], hex3[2] + off3[2]],
    ];
    // const rgb1 =[255,0,0];
    // const rgb2 =[0,0,255];
  ...
  generateRandomColor() {
    ...
    if (
      Array.isArray(this.initrgb) &&
      this.initrgb.length > 0 &&
      Array.isArray(this.initrgb[0]) &&
      this.initrgb[0].length > 0 &&
      Array.isArray(this.initrgb[0][0]) &&
      this.initrgb[0][0].length > 0
    ) {
    ...
    if (
      ...
      this.initrgb.length > 0 &&
      Array.isArray(this.initrgb[0]) &&
      this.initrgb[0].length === 3
    ) {
      this.rgb = [...this.initrgb[this.randomInt(0, this.initrgb.length)]];
    }

    // Something is not right, default to white bg.
    if (!Array.isArray(this.initrgb) && this.initrgb.length !== 3)
      return `#ffffff`;
    ...
    this.rgb.forEach((v, idx, arr) => {
      v = Number.isInteger(arr[idx])
        ...
        ? this.randomInt(arr[idx][0], arr[idx][1])
        : null;
...
ReactDOM.render(<BgComponent />, document.getElementById("bg"));
